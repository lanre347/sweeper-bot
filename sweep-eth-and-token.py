...
# === MAIN FUNCTION (ETH) ===
def send_all_eth_from_all_wallets(max_fee_eth):
    private_keys = load_private_keys()
    receiver_address = load_receiver()

    while True:
        print("\nStarting new ETH cycle...")
        for pk in private_keys:
            account = Account.from_key(pk)
            sender = account.address
            balance_wei = get_eth_balance(sender)

            if balance_wei > 21000:  # enough to cover gas
...
if __name__ == "__main__":
    choice = input("Type 'eth' to send ETH or 'token' to send tokens: ").strip().lower()
    max_fee_eth = float(input("Enter max gas fee in ETH (e.g. 0.0001): "))

    if choice == "token":
        send_all_tokens_from_all_wallets(max_fee_eth)
    elif choice == "eth":
        send_all_eth_from_all_wallets(max_fee_eth)
    else:
        print("Invalid choice. Please type 'eth' or 'token'.")
